...
try {
    ...
    if (isset($data['jobs']) && is_array($data['jobs'])) {
        ...
        foreach ($data['jobs'] as $job) {
            $job_link = isset($job['job_link']) ? htmlspecialchars($job['job_link']) : null;
            $job_title = isset($job['job_title']) ? htmlspecialchars($job['job_title']) : null;
            $company = isset($job['company']) ? htmlspecialchars($job['company']) : null;
            $country = isset($job['country']) ? htmlspecialchars($job['country']) : null;
            $city = isset($job['city']) ? htmlspecialchars($job['city']) : null;
            $county = isset($job['county']) ? htmlspecialchars($job['county']) : null;
            $remote = isset($job['remote']) ? htmlspecialchars($job['remote']) : null;
        ...
        if (empty($items)) {
            ...
            echo json_encode([
                "error" => "No valid jobs found in payload",
                "code" => 400
            ]);
            exit;
        }
        ...
        echo json_encode(["success" => "Data successfully inserted into Solr", "count" => count($items)]);
    } else {
