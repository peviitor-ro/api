...
try {
    ...
    if (!isset($input['id']) || !isset($input['scraper'])) {
        http_response_code(400);
        echo json_encode(["error" => "Missing id or scraper"]);
        exit;
    }
    ...
    $payload = json_encode([
        [
            "id" => $id,
            "scraper" => ["remove" => $scraper]
        ]
    ]);

    error_log("FIRME SCRAPER DELETE URL: $url");
    ...
    error_log("FIRME SCRAPER DELETE FAILED: " . $e->getMessage());
